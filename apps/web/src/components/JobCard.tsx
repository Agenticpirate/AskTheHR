...
import { hoverLift, itemRise } from "@/lib/motion";

export function JobCard({ job }: { job: Job }) {
  const reduce = useReducedMotion();
  return (
    <motion.div variants={itemRise} whileHover={reduce ? undefined : hoverLift}>
      <Link
        to={jobPath(job)}
      ...
      >
        ...
        <div className="mt-3 text-[11px] text-muted-foreground">{formatPosted(job.posted_at)}</div>
      </Link>
    </motion.div>
  );
}
